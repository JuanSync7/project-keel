...
| `review_docs.tool.md` | `scripts/jobs/review_docs.py` | `read-only` — the deterministic documentation review as JSON: stale `updated:` stamps (never earlier than the last commit; today when modified), backticked paths that resolve to nothing, and every `## What ships here` row with its `Not for` cell; exit 0 as a report, `--strict` exits 1 on stale stamps only | Not for gating from an agent — `run_make_target.tool.md` with `check-docs` runs this doer strictly alongside `check_structure`; not for judging a roster cell — it reports the cell, `agents/doc_reviewer` judges it; not for structure, links or citations — `structure_check.tool.md` |
...
| `structure_check.tool.md` | `scripts/check_structure.py` | `read-only` — runs the 3.6-safe conventions gate (checks A–V: frontmatter and unique ids, labelled dirs, package and private-import boundaries, tool/agent governance, project facts, practice boundaries, ruleset/twin/help/catalogue parity, cross-references, rosters, practice mechanisms, policy reachability, writer rerun declarations); prints `WARN`/`ERROR` lines and exits 1 on any error | Not for owner gaps across the corpus — its accountability WARN covers only `kind: tool`/`agent` docs read from disk; `accountability_report.tool.md` lists every unowned node of the built corpus. Not for a pass/fail on any other make target, which is `run_make_target.tool.md`. It builds nothing: the corpus is `build_corpus.tool.md` |
