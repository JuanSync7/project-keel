...
def _answer_prompt(question, nodes):
    ctx = "\n".join(
        "- [%s] %s (%s): %s"
        % (
            n.get("summary_source") or "gap",
            n.get("node_id"),
            n.get("path"),
            n.get("summary") or n.get("text_excerpt", ""),
        )
        for n in nodes
    )
    return (
...
def _retrieve(state):
    ...
    return {
        "citations": citations,
        "prompt": _answer_prompt(state["question"], visible),
    }
...
def answer(
    question, *, execute=False, model=None, corpus=None, max_nodes=8, runtime=None
):
