...
def main(argv=None):
    ...
    ap.add_argument("--model", default=None, help="model name from the models/ registry")
    ap.add_argument("--gate", default="verify",
    ...
    ap.add_argument("--corpus", default=None, help="corpus path (default: <root>/wiki/corpus.json)")
    ap.add_argument("--root", default=None, help="repo root to refactor (default: this repo)")
    ap.add_argument("--runtime", default=None, help="runtime engine name (default: inprocess)")
    ap.add_argument("--json", action="store_true", help="emit the report as JSON")
    args = ap.parse_args(argv)

    report = refactor(args.practice, execute=args.execute, model=args.model,
                      gate=args.gate, max_nodes=args.max_nodes, corpus=args.corpus,
                      root=args.root, runtime=args.runtime)
    ...
    if args.json:
        ...
        sys.stdout.write(
            ...
            % (report.practice, mode, "green" if report.baseline_green else "red",
               len(report.candidates), len(report.refactored), len(report.skipped)))
