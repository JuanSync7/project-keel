...
| `check_structure.py` | The stdlib, 3.6-safe conventions gate — checks A–V: frontmatter, taxonomy, package boundary, module headers, tool-agent binding, project facts, agent-rules symlinks, gate-tier practices, ruleset, twin, help and catalogue parity, cross-references, rosters, practice mechanisms, policy reachability, writer rerun declarations; exit 1 on any error, warnings never fail | The corpus graph — `jobs/check_corpus.py`; advisory smells — `check_generic.py` and `check_practices.py`; owner gaps across corpus nodes — `accountability_report.py`; external code-doc drift — `cdmon_sync.py`; the published contracts — `agent_surface/generate_aad_schema.py --check` | `make check` (from `check-all` and `verify`); pre-commit hook `structure`; `agents/tools/structure_check.tool.md` (index_enforcer); `jobs/build_corpus.py` imports its `CODE_ROOTS` and `IGNORE_DIRS` |
| `hooks/` | Event-triggered doers (§7): `hooks/on_stop_triage.py` hands an event payload (argument or stdin) to `agents.triage`; `hooks/on_stop_doc_review.py` runs the doc reviewer's dry-run when a turn ends and prints its one-line report (fired by `.claude/settings.json`); both dry-run unless `--execute`, model named by `--model` from `models/`; an unavailable model is a stated skip at exit 0 | Time-triggered work — `jobs/`; the trigger itself (which event fires it) — the ecosystem adapter (`.pre-commit-config.yaml`, `.github/workflows/`, `.claude/settings.json`); the reasoning — `agents/triage` | `.claude/settings.json` fires `hooks/on_stop_doc_review.py` on `Stop`; nothing in the tree fires `hooks/on_stop_triage.py` yet |
...
| `doc_review.py` | Thin CLI over `agents.doc_reviewer.review()`: gathers the deterministic doc findings, retrieves the style rules, gates a green `check-docs` baseline and (only with `--execute`) proposes and applies one gated edit per chunk; dry-run by default; `--json` report; an unavailable model is a stated skip at exit 0 | The reasoning, prompt or model choice — `agents/doc_reviewer` and `models/`; the deterministic facts — `jobs/review_docs.py`; applying one edit with rollback — `apply_refactor.py`; the trigger — `.claude/skills/doc-review`, `hooks/on_stop_doc_review.py` | `make doc-review`, `make doc-review-apply`; the `doc-review` skill |
...
- `check_structure.py` — conventions validator (checks A–V); `make check`.
