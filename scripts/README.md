...
| `review_docs.py` | The deterministic documentation review: freshness (§1 — every governed Markdown file's `updated:` is no earlier than its last commit, and a file modified in the working tree carries today's date) and the advisory for backticked repository paths that resolve to nothing; text or `--json`; exit 0 as a report, `--strict` exits 1 on stale stamps only | Structure, links or citations — `check_structure.py` (checks A–T) reads those without git; owner gaps — `accountability_report.py`; anything needing a model — a doc-review agent, not this | `make advise`; `tests/integration/test_doc_freshness.py` (as the gate, via `--strict` semantics) |
