...
def _branching(calls):
    ...
    return Plan(
        ...
        "gate",
        (
            Step("gate", READ_ONLY, gate),
            Step("build", WRITES, build),
            Step("report", READ_ONLY, report),
            Step("fill", MODEL_CALL, fill),
        ),
        (
...
@pytest.mark.parametrize(
    "label,init,execute", _SCENARIOS, ids=[s[0] for s in _SCENARIOS]
)
...
def test_durable_run_equivalence():
    ...
    for engine in ENGINES:
        ...
        res = get_runtime(engine).run(
            plan, {}, execute=True, checkpointer=cp, run_key="k"
        )
...
def test_human_in_the_loop_equivalence():
    ...
    for engine in ENGINES:
        cp = MemoryCheckpointer()
        p = get_runtime(engine).run(
            _approval_plan(), {}, execute=True, checkpointer=cp, run_key="h"
        )
        r = get_runtime(engine).run(
            _approval_plan(),
            {},
            execute=True,
            checkpointer=cp,
