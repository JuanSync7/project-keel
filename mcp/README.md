...
|--------|--------|-------|--------------|---------|
| `action_server.py` | `keel-actions` (state-changing, dry-run) | `rebuild_index` | `scripts/jobs/rebuild_index.py` | Not for answering questions or reading the corpus — that is `qa_server.py` (`wiki_answer`); not for the wire or the tool registry — that is `protocol.py`. Its one tool shells out to the doer and regenerates the README-title index (`wiki/INDEX.md` by default) — the worked example of a state-changing tool; nothing else in the tree reads that file, and the nightly schedule now runs `scripts/jobs/review_docs.py` instead. Never `wiki/corpus.json` (that is `scripts/jobs/build_corpus.py` via `make site-data`); it writes only with `execute: true` and refuses an `out` that resolves outside the indexed tree. |
